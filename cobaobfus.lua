...
local function CreateCloudyPanel()
    ...
    main.Position = UDim2.new(0.5, -110, 0, 14)
    ...
    stroke.Color = Color3.fromRGB(165, 185, 215)
    ...
    bgGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0.0, Color3.fromRGB(26, 32, 45)),
        ColorSequenceKeypoint.new(1.0, Color3.fromRGB(12, 15, 21))
    })
    ...
    titleGradient.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(1.0, Color3.fromRGB(155, 168, 185))
    })
    ...
    statsBar.Position = UDim2.new(0, 8, 0, 30)
    ...
    local function makeStatPill(tag, defaultVal, colorHex)
        ...
        label.RichText = true
        label.Text = string.format("<font color='#8A99AD'>%s</font> <font color='%s'>%s</font>", tag, colorHex or "#E2E8F0", tostring(defaultVal))
        label.Parent = pill

        return label
    end

    local pingLabel = makeStatPill("PING", "0ms", "#67E8F9")
    ...
    local updateThread = task.spawn(function()
        while gui and gui.Parent do
            pcall(function()
                if PingMonitor and PingMonitor.Update then
                    PingMonitor:Update()
                end
                local ping = getPing()
                ...
                local notifColor = notifCount > 0 and "#7DD3FC" or "#94A3B8"
                notifLabel.Text = string.format("<font color='#8A99AD'>NOTIF</font> <font color='%s'>%d</font>", notifColor, notifCount)

                local fpsColor = fps >= 45 and "#F1F5F9" or (fps >= 25 and "#CBD5E1" or "#F87171")
                fpsLabel.Text = string.format("<font color='#8A99AD'>FPS</font> <font color='%s'>%d</font>", fpsColor, fps)
            end)
            task.wait(0.5)
        end
    end)
